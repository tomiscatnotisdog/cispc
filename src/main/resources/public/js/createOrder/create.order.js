layui.use(['table','layer'],function(){
    ...
    var tableIns = table.render({
        id:'createOrderTable'
        // 容器元素的ID属性值
        ,elem: '#createOrderList'
        // 容器的高度 full-差值
        ,height: 'full-125'
        // 单元格最小的宽度
        ,cellMinWidth:95
        // 访问数据的URL（后台的数据接口） 设置flag参数，表示查询的事客户开发计划数据
        ,url: ctx + '/create_order/myList?flag=1'
        // 开启分页
        ,page: true
        // 默认每页显示的数量
        ,limit:10
     ...
     * 行工具栏监听
     */
    table.on('tool(createOrder)', function (data) {


        // 判断类型
        if (data.event == "comple") { // 更新订单信息
            // 得到的创建订单的ID
            ...
            // 删除操作
            // 弹出确认框，询问用户是否确认删除
            layer.confirm('您确定已经完成选中的订单了吗？',{icon:3, title:"完成该条订单"}, function (index) {
                // 关闭确认框
                layer.close(index);
                ...
                $.ajax({
                    type:"post",
                    url:ctx + "/create_order/completeOrder",
                    data:{
                        ids:data.data.id
                    },
     ...
     * 监听头部工具栏
     */
    table.on('toolbar(createOrder)',function (data){
        if(data.event =="complete"){//发起跑腿订单
            //打开发起或更新跑腿订单的页面
            completeOrder();
        }
    });
