...
layui.use(['table','layer'],function(){
    ...
    var tableIns=  table.render({
        id : "saleChanceList",
        elem: '#frendList',
        url : ctx +'/join/list?joinMan='+joinMan,
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
    ...
    //  发起活动
    function openwindow(id,joinMan){
        var title = "<h2>添加评论</h2>"
        var url = ctx + "/join/addcomment?id="+id+"&joinMan="+joinMan;
    ...
    table.on('tool(frend)',function (data) {
        ...
        if (data.event == "comment"){
            ...
            var  arr1 = item.split("=");
            var joinMan = arr1[1];
            openwindow(id,joinMan);
        }else if (data.event == "tousu"){
            layer.confirm('确定开始评价吗',{icon:3,title:"参加聚会"},function (index) {
        ...
        } else if (data.event == "unjoin"){ //删除操作
            layer.confirm('确定取消参加这个活动吗',{icon:3,title:"取消参加活动"},function (index) {
                layer.close(index);//关闭确认框
                   console.log(data.data.makeFrendId);
                var arr = document.cookie.split("; ");
                item = arr[3];
                var  arr1 = item.split("=");
                var joinMan = arr1[1];
