...
/*   $.ajax({
   ...
   form.on('submit(addComment)',function (data) {
       ...
       /* let id = data.data.makeFrendId;*/
      //   发送ajax
       var url = ctx + "/join/updatecomment";
       var arr = document.cookie.split("; ");
       item = arr[3];
       var  arr1 = item.split("=");
       var joinman = arr1[1]
      /* var joinId = $("[name='id']").val()
       if (joinId != null && joinId != ''){
           var url = ctx + "/join/update/id="+id;
           console.log(joinId)
       }*/
       $.post(url,{comment:data.field.comment,
                        id:data.field.id,joinMan:joinman},function (result) {
           if (result.code == 200){
